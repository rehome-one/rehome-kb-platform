...
def _register_webhook(
    ...
    resp = kb_client.post(
        "/api/v1/webhooks",
        json={"url": receiver_url, "events": events, "description": "e2e"},
...
async def test_publish_article_delivers_webhook_with_hmac(
    ...
    wh = _register_webhook(kb_client, m2m_token, receiver.url, events=["article.published"])
...
async def test_chat_escalated_delivers_webhook(
    ...
    wh = _register_webhook(kb_client, m2m_token, receiver.url, events=["chat.escalated"])
    cleanup["webhook_ids"].append(UUID(wh["id"]))
    secret = wh["secret"]
